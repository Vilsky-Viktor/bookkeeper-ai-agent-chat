...
from app.auth import bearer_token, require_uid
...
class TestBearerToken:
    ...
    def test_no_prefix_returned_unchanged(self):
        # removeprefix is a no-op when the prefix isn't present — bearer_token has no
        # validation of its own (require_uid, run first in practice, already rejected
        # anything without "Bearer ").
        assert bearer_token(authorization="already-stripped") == "already-stripped"
